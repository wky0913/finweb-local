...
class DataLoaderSingle(object):
    def __init__(self,code,date,dic={}):
        if USE_LOCAL_DATA:
            self.stocks = []
            self.df = dic[code]
            self.dfn = 0
    ...
    def get_index_price(self):
        if USE_LOCAL_DATA:
            return self.df[self.date, 'PRICE']
        else:
...
class DataLoaderSingleCode(object):
    def __init__(self, code, dates, dic={}):
        self.dic = dic
        self.code = code
        if USE_LOCAL_DATA:
            self.dates = dates
    ...
    def get_pes(self):
        ...
        for date in self.dates:
            dls = DataLoaderSingle(self.code,date,self.dic)
...
class DataLoader(object):
    def __init__(self, codes, dates, dic={}):
        self.dic = dic
        self.codes = codes
        self.dates = dates
    def get_index_dic(self):
        ...
        for code in self.codes:
            dl = DataLoaderSingleCode(code, self.dates,self.dic)
